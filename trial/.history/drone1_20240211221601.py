...
def upload():
    coordinate = str(x_pos)+","+str(y_pos)+","+str(z_pos)+","+str(roll)+","+str(pitch)+","+str(yaw)
    data = database.child("menu").child("tello").child("data").child("drone1").get()
    index = len(data.val())
    database.child("menu").child("tello").child("data").child("drone1").child(index).push(coordinate)
