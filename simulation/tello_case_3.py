...
def run(database, drone_id):
    ...
    while True:
        ...
        if distance < safe_distance:
            if drone_id == 1:
                # Drone 1: Naik jika terlalu dekat dengan Drone 2
                if tello.get_height() < safe_altitude:
                    tello.move_up(10)
            else:
                # Drone 2: Turun jika terlalu dekat dengan Drone 1
                if tello.get_height() > safe_altitude:
                    tello.move_down(10)

        # Putar drone sesuai arah
        tello.rotate_clockwise(30) if drone_id == 1 else tello.rotate_counter_clockwise(30)
